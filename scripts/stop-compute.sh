#!/bin/bash
# Stop Compute / Cleanup Script (best-effort)
set -euo pipefail
...
case "$ACTION" in
  ...
  stop-aura)
    if [[ -z "${AURA_CLIENT_ID:-}" || -z "${AURA_CLIENT_SECRET:-}" || -z "${AURA_INSTANCE_ID:-}" ]]; then
      echo "Aura credentials or instance ID not provided; skipping"
      exit 0
    fi
    echo "Attempting Aura suspend (best-effort) for ${AURA_INSTANCE_ID}"
