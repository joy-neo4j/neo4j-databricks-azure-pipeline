...
output "databricks_workspace_id" {
  description = "ID of the Databricks workspace (only available if create_databricks_workspace=true)"
  value       = var.create_databricks_workspace ? module.azure_databricks[0].workspace_id : null
}

output "databricks_workspace_url" {
  description = "URL of the Databricks workspace (only available if create_databricks_workspace=true, otherwise use DATABRICKS_HOST)"
  value       = var.create_databricks_workspace ? module.azure_databricks[0].workspace_url : var.databricks_host
}

output "databricks_workspace_name" {
  description = "Name of the Databricks workspace (only available if create_databricks_workspace=true)"
...
output "deployment_info" {
  ...
  value = {
    ...
    databricks_workspace = var.create_databricks_workspace ? module.azure_databricks[0].workspace_name : "existing (via DATABRICKS_HOST)"
